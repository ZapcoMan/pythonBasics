...
# 定义一个十六进制格式的shellcode字符串，并将其转换为字节序列
...
def rot13(message):
    ...
    ROT13是一种简单的字母替换密码，将字母表中的每个字母替换为距离它13位的字母。
    大写字母A-M会被替换为N-Z，N-Z会被替换为A-M；小写字母a-m会被替换为n-z，n-z会被替换为a-m。
    非字母字符保持不变。

    参数:
        message (str): 需要进行ROT13处理的字符串
