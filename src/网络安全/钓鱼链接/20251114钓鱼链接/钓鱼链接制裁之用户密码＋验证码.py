...
if __name__ == "__main__":
    # 生成虚假凭证
    fake_credentials = generate_fake_credentials(10)  # 减少数量用于测试
    ...
    for i, credential in enumerate(fake_credentials):
        try:
            print(f"\n--- 处理第 {i + 1} 个用户 ---")
            if process_complete_request(credential):
                success_count += 1
            print(f"--- 第 {i + 1} 个用户处理完成 ---")
        except Exception as e:
            print(f"❌ 用户 {i + 1} 处理异常: {e}")
